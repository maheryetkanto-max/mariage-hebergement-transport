...
export type NamedPerson = { firstName: string; lastName: string; kind: "adult" | "child"; gender: "homme" | "femme" }
export function personLabel(person: NamedPerson) { return `${person.firstName.trim()} ${person.lastName.trim()}${person.kind === "child" ? " (enfant)" : ""}${person.gender ? ` · ${person.gender}` : ""}` }
export function NamedPeople({ value, onChange, title }: { value: NamedPerson[]; onChange: (value: NamedPerson[]) => void; title: string }) {
  ...
  return <div className="space-y-3 rounded-2xl border border-[#6D1925]/10 bg-white/70 p-4">
    ...
    {value.map((person, index) => <div key={index} className="grid gap-2 rounded-xl bg-[#FFF7E9] p-3 sm:grid-cols-[1fr_1fr_auto_auto_auto]">
      ...
      <select className="rounded-lg border border-[#6D1925]/15 bg-white p-2 text-sm" aria-label={`Sexe personne ${index + 1}`} value={person.gender ?? "femme"} onChange={(event) => update(index, { gender: event.target.value as NamedPerson["gender"] })}><option value="femme">Femme</option><option value="homme">Homme</option></select>
    ...
    <button type="button" onClick={() => onChange([...value, { firstName: "", lastName: "", kind: "adult", gender: "femme" }])} className="min-h-11 rounded-xl border border-[#6D1925]/20 px-4 text-sm font-semibold text-[#6D1925]">+ Ajouter une personne</button>
  </div>
}
