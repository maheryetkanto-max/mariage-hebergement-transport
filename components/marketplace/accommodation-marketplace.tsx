...
export function AccommodationMarketplace() {
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && <AccommodationForm onClose={() => setShowForm(false)} onPublished={(url) => { setManageUrl(url); window.localStorage.setItem("last-offer-group-link", url); setShowForm(false); setArrival(""); setDeparture(""); setCity(""); setPeople(1); setChildrenOnly(false); setPetsOnly(false); window.setTimeout(() => document.getElementById("accommodation-offers")?.scrollIntoView({ behavior: "smooth", block: "start" }), 30) }} source={offerSource} />}
...
function AccommodationForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      onPublished(`/groupe?type=accommodation&offre=${offerId}&gestion=${groupToken}`)
    } catch (error) {
      console.error(error)
